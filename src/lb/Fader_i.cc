...
void LB_Fader_i::do_run(void)
{
  ...
  if ((tolevel - fromlevel) > 0)
    while ((double(steps) * delta) < (tolevel - fromlevel))
    {
      steps++;
    }
...
void LB_Fader_i::sendEvent(const LB::Event& evt)
{
  ...
  case LB::event_fader_run:
    handle= &this->run_listeners;
    break;

  case LB::event_fader_stop:
    handle= &this->stop_listeners;
    break;

  case LB::event_fader_complete:
    handle= &this->complete_listeners;
    break;

  case LB::event_fader_source:
    handle= &this->source_listeners;
    break;
  } // switch
...
void LB_Fader_i::removeListener(GSList **list, CORBA::Long id)
{
  ListenerRecord *r;
  GSList *l= *list;
