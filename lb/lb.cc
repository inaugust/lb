...
void normalize_cues(const LB::Cue& incue1,
                    const LB::Cue& incue2,
                    LB::Cue& outcue1,
                    LB::Cue& outcue2)
{
  ...
  for (int i= 0; i < numins; i++)
  {
    ...
    g_hash_table_insert(hash2,
                        const_cast<char *>(incue2.ins[i].name._ptr),
                        (gpointer)(i + 1));
    pos++;
  }
  ...
  for (int i= 0; i < numins; i++)
  {
    ...
    g_hash_table_insert(hash1,
                        const_cast<char *>(incue1.ins[i].name._ptr),
                        (gpointer)(i + 1));
  }
      ...
      for (int a2= 0; a2 < numattr2; a2++)
      {
        ...
        for (int a1= 0; a1 < numattr1; a1++)
        {
          if (incue1.ins[pos1].attrs[a1].attr == LB::attr_level)
          {
            ...
            outcue1.ins[i].attrs[a2].value=                   // deep copy
                                            incue1.ins[pos1].attrs[a1].
                                               value;         // or so i'm told
...
getRootNamingContext(CORBA::ORB_ptr orb)
{
  ...
  catch (...)
  {
    fprintf(
      stderr,
      "Caught a system exception while resolving the naming service.\n");
    exit(1);
  }
  return rootContext;
...
} // recursive_unbind_and_destroy_help
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    PortableServer::ObjectId_var myLB_Lightboard_iid= poa->activate_object(
      myLB_Lightboard_i);
